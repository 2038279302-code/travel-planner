...
router.get('/', async (req, res, next) => {
  try {
    ...
    res.json(await ActivityRepo.byTrip(tripId));
  } catch (err) {
    next(err);
  }
});
...
router.post('/', validateBody(activitySchema), async (req, res, next) => {
  try {
    ...
    const activity = await ActivityRepo.create(tripId, {
...
router.patch('/reorder', validateBody(activityReorderSchema), async (req, res, next) => {
  try {
    ...
    const updated = await ActivityRepo.reorder(normalized);
    res.json(updated);
  } catch (err) {
    next(err);
  }
...
/** 更新行程项 */
router.put('/:id', validateBody(activityUpdateSchema), async (req, res, next) => {
  try {
    const trip = req.trip!;
    const d = { ...req.body };
    if (d.dayDate) {
    ...
    if (d.startTime !== undefined || d.endTime !== undefined) {
      const existing = (await ActivityRepo.find(req.params.id)) as Activity | null;
    ...
    const activity = await ActivityRepo.update(req.params.id, d);
...
router.delete('/:id', async (req, res, next) => {
  try {
    await ActivityRepo.remove(req.params.id);
    res.status(204).end();
  } catch (err) {
    next(err);
  }
